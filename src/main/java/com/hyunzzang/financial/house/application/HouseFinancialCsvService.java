...
public class HouseFinancialCsvService {
  ...
  private Map<Integer, Institution> makeInstitutionMap(String[] row) {
    ...
    for (int i = 2; i < row.length; i++) {
      ...
      resMap
          .put(i, Institution.ByInstitutionTypeBuilder().institutionCode(institutionCode).build());
    }

    return resMap;
  }

  private List<HouseFund> makeHouseFundToList(final List<String[]> rowList,
      final Map<Integer, Institution> resMap) {
  ...
  private List<HouseFund> makeHouseFund(final int rowNo, final String[] row,
      final Map<Integer, Institution> resMap) {
